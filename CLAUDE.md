...
The workflow's `actions/checkout` step authenticates with `secrets.BOT_PUSH_TOKEN` — a fine-grained PAT owned by the repo admin (`anduaura`). Required because the `main` branch ruleset requires PRs even from admin (the "Require pull request" rule isn't bypassable for direct push, only the rules *within* a PR are). The PAT needs **Contents: read/write** + **Pull requests: read/write** on this repo.

To work within the PR-required ruleset, the workflow doesn't push to `main` directly. It pushes to a unique side branch (`bot/refresh-YYYYMMDD-HHMMSS`), opens a PR via `gh pr create`, then merges with `gh pr merge --admin --rebase --delete-branch`. The `--admin` flag bypasses the approval count and the status-check requirement — the latter matters because the `tests` workflow has a `paths:` filter that excludes `shows.json`, so it doesn't run on data-only PRs. Auto-delete-head-branches is enabled at the repo level so each side branch disappears after merge.

If the PAT ever needs to be rotated, update the secret value at `Settings → Secrets and variables → Actions → BOT_PUSH_TOKEN` and confirm the new PAT has both Contents and Pull requests permissions.
